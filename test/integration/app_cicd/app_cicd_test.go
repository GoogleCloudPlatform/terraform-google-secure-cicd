...
func TestAppCICDExample(t *testing.T) {
	...
	appCICDT.DefineVerify(func(assert *assert.Assertions) {
		...
		gcbCI := gcloud.Run(t, fmt.Sprintf("builds triggers describe %s --project %s --region", sourceTriggerName, projectID, primaryLocation))
		...
		for _, cdTrigger := range cdTriggers {
			gcbCD := gcloud.Run(t, fmt.Sprintf("builds triggers describe %s --project %s --region", cdTrigger, projectID, primaryLocation))
