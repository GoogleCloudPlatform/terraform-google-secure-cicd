...
func TestCloudBuildPrivatePoolExample(t *testing.T) {
	...
	cloudBuildPrivatePoolT.DefineVerify(func(assert *assert.Assertions) {
		...
		// VPN Tunnels
		vpnTunnels := [6]string{"cloudbuild-to-gke-cbpp-vpc-dev-remote-0", "cloudbuild-to-gke-cbpp-vpc-dev-remote-1", "cloudbuild-to-gke-cbpp-vpc-prod-remote-0", "cloudbuild-to-gke-cbpp-vpc-prod-remote-1", "cloudbuild-to-gke-cbpp-vpc-qa-remote-0", "cloudbuild-to-gke-cbpp-vpc-qa-remote-1"}
